...
CKido::CKido(CHost* pHost, CDataManagerBalance* pBalanceData, F4PACKET::SPlayerInformationT* pInfo, CAnimationController* pAniInfo, CBallController* pBallInfo,
	DHOST_TYPE_UINT32 playerNumber) : CCharacter(pHost, pBalanceData, pInfo, pAniInfo, pBallInfo, playerNumber)
{
	...
	{
		float a = m_pBalanceData->GetValue("VPowerRebound_Condition_Radius_Kido_Special");
		float b = m_pBalanceData->GetValue("VPowerRebound_Condition_Radius_Kido");
	...
	{
		passiveValue = m_pBalanceData->GetValue("VPowerRebound_Condition_Radius_Kido_Special");
	}
	else
	{
		passiveValue = m_pBalanceData->GetValue("VPowerRebound_Condition_Radius_Kido");
	}
...
DHOST_TYPE_BOOL CKido::InitialPassiveDataSet(const F4PACKET::play_c2s_playerPassiveDataSet_data* pInfo)
{

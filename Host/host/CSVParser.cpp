...
extern "C" {
	...
	CsvParser* CsvParser_new(const char* filePath, const char* delimiter, int firstLineIsHeader)
	{
		...
		{
			auto filePathLen = strlen(filePath);
			csvParser->filePath_ = (char*)malloc((filePathLen + 1));
			// memset(csvParser->filePath_, 0, sizeof(filePathLen+1));
			// strcpy(csvParser->filePath_, filePath);
#if (defined(WIN32) || defined(_WIN32) || defined(__WIN32__))
			strncpy_s(csvParser->filePath_, filePathLen + 1, filePath, filePathLen);
#else
			strncpy(csvParser->filePath_, filePath, filePathLen + 1);
#endif
		}
...
#if (defined(WIN32) || defined(_WIN32) || defined(__WIN32__))
			strncpy_s(csvParser->csvString_, csvStringLen + 1, csvString, csvStringLen);
#else
			strncpy(csvParser->csvString_, csvString, csvStringLen);
#endif
		}
		return csvParser;
	}
	...
	CsvRow* _CsvParser_getRow(CsvParser* csvParser)
	{
		...
		if (!csvParser->fromString_)
		{
			...
			{
#if (defined(WIN32) || defined(_WIN32) || defined(__WIN32__))
				fopen_s(&csvParser->fileHandler_, csvParser->filePath_, "r+");
#else
...
#if (defined(WIN32) || defined(_WIN32) || defined(__WIN32__))
					// strcpy_s(errMsg, 1024, "");
					sprintf_s(errMsg, 1024, "Error opening CSV file for reading: %s : %s", csvParser->filePath_, errnomsg);
#else
					// strncpy(errMsg, "", 1);
					snprintf(errMsg, 1024, "Error opening CSV file for reading: %s : %s", csvParser->filePath_, errnomsg);
#endif
					_CsvParser_setErrorMessage(csvParser, errMsg);
					free(errMsg);
					return NULL;
...
#if (defined(WIN32) || defined(_WIN32) || defined(__WIN32__))
				strcpy_s(csvRow->fields_[fieldIter], currFieldCharIter + 1, currField);
#else
				strncpy(csvRow->fields_[fieldIter], currField, currFieldCharIter + 1);
#endif
				free(currField);
				csvRow->numOfFields_++;
				if (currChar == '\n')
	...
	void _CsvParser_setErrorMessage(CsvParser* csvParser, const char* errorMessage)
	{
		...
		auto errMsgLen = strlen(errorMessage);
...
#else
		strncpy(csvParser->errMsg_, errorMessage, errMsgLen);
#endif
	}

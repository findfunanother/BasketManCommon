...
CHayato::CHayato(CHost* pHost, CDataManagerBalance* pBalanceData, F4PACKET::SPlayerInformationT* pInfo, CAnimationController* pAniInfo, CBallController* pBallInfo,
	DHOST_TYPE_UINT32 playerNumber) : CCharacter(pHost, pBalanceData, pInfo, pAniInfo, pBallInfo, playerNumber)
{
	...
	{
		passiveValue = m_pBalanceData->GetValue("VPassive_Burst_Hayato_Special");
	}
	else
	{
		passiveValue = m_pBalanceData->GetValue("VPassive_Burst_Hayato");
	}
...
DHOST_TYPE_BOOL CHayato::InitialPassiveDataSet(const F4PACKET::play_c2s_playerPassiveDataSet_data* pInfo)
{

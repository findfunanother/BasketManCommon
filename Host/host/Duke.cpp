...
CDuke::CDuke(CHost* pHost, CDataManagerBalance* pBalanceData, F4PACKET::SPlayerInformationT* pInfo, CAnimationController* pAniInfo, CBallController* pBallInfo,
	DHOST_TYPE_UINT32 playerNumber) : CCharacter(pHost, pBalanceData, pInfo, pAniInfo, pBallInfo, playerNumber)
{
	...
	{
		float a = m_pBalanceData->GetValue("VPassive_JumpShotProbability_Duke_Special");
		float b = m_pBalanceData->GetValue("VPassive_JumpShotProbability_Duke");
	...
	{
		passiveValue = m_pBalanceData->GetValue("VPassive_JumpShotProbability_Duke_Special");
	}
	else
	{
		passiveValue = m_pBalanceData->GetValue("VPassive_JumpShotProbability_Duke");
	}
...
DHOST_TYPE_BOOL CDuke::InitialPassiveDataSet(const F4PACKET::play_c2s_playerPassiveDataSet_data* pInfo)
{

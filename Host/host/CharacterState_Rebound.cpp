...
CCharacterState_Rebound::CCharacterState_Rebound(CCharacter* pInfo) : CCharacterState(pInfo)
{
	Init();
}

CCharacterState_Rebound::~CCharacterState_Rebound()
{
	
}

void CCharacterState_Rebound::OnEnter(void* pData)
{
	Init();
	m_pOwner->InitCharacterStateStartAnimationPosition();
}

void CCharacterState_Rebound::OnUpdate(DHOST_TYPE_FLOAT gameTime, DHOST_TYPE_FLOAT timeDelta, DHOST_TYPE_FLOAT elapsedTime, JOVECTOR3 ballPos)
{
	switch (m_pOwner->GetCharacterStateActionType())
	{
		case F4PACKET::ACTION_TYPE::action_rebound:
		case F4PACKET::ACTION_TYPE::action_tapOut:
		case F4PACKET::ACTION_TYPE::action_tapPass:
		case F4PACKET::ACTION_TYPE::action_jumpBallTapOut:
		{
			...
			m_pOwner->SetCharacterStateActionPosition(sResultPos.fX, sResultPos.fY, sResultPos.fZ, "CCharacterState_Rebound::OnUpdate action_rebound");

#ifdef TEXT_LOG_FILE
			//LOGGER->Log("[CCHARACTER_STATE_REBOUND] ActionName : %s, AnimTime : %f, Pos x : %f, z : %f, DestYaw : %f", F4PACKET::EnumNameACTION_TYPE(m_pOwner->GetCharacterStateActionType()), animation_time, sResultPos.fX, sResultPos.fZ, yawDest);
#endif
		}
		break;
		default:
...
void CCharacterState_Rebound::OnMessage(DHOST_TYPE_INT32 packetID, void* pData, DHOST_TYPE_GAME_TIME_F time)
{
	switch ((F4PACKET::PACKET_ID)packetID)
	{
		case F4PACKET::PACKET_ID::play_c2s_playerRebound:
		{
			auto packet_data = (CFlatBufPacket<F4PACKET::play_c2s_playerRebound_data>*)pData;
			...
			m_pOwner->SetCharacterStateAction(*packet_data->GetData()->playeraction(), F4PACKET::ACTION_TYPE::action_rebound, time);
...
#ifdef TEXT_LOG_FILE
			//LOGGER->Log("[CCHARACTER_STATE_REBOUND] PacketName : %s, AnimID : %d, AnimationDuration : %f", F4PACKET::EnumNamePACKET_ID((F4PACKET::PACKET_ID)packetID), m_pOwner->GetCharacterStateAnimationIndex(), m_AnimationDuration);
			//LOGGER->Log("[CCHARACTER_STATE_REBOUND] StartYaw : %f, StartPos x : %f, z : %f", m_pOwner->GetStartYaw(), m_pOwner->GetStartPosition().fX, m_pOwner->GetStartPosition().fZ);
			//LOGGER->Log("[CCHARACTER_STATE_REBOUND] PacketYaw : %f, PacketPos x : %f, z : %f, m_SlideTimePosition : %f", packet_data->GetData()->playeraction()->yawlogic(), packet_data->GetData()->playeraction()->positionlogic().x(), packet_data->GetData()->playeraction()->positionlogic().z(), m_SlideTimePosition);
			//LOGGER->Log("[CCHARACTER_STATE_REBOUND] m_SlideYaw : %f, m_SlidePosition x : %f, z : %f, Mirror : %d", m_SlideYaw, m_SlidePosition.fX, m_SlidePosition.fZ, m_Mirror);
#endif
		}
		break;
		case F4PACKET::PACKET_ID::play_c2s_playerTapOut:
		{
			auto packet_data = (CFlatBufPacket<F4PACKET::play_c2s_playerTapOut_data>*)pData;
			...
			m_pOwner->SetCharacterStateAction(*packet_data->GetData()->playeraction(), F4PACKET::ACTION_TYPE::action_tapOut, time);
			m_pOwner->SetCharacterStateAnimationIndex(packet_data->GetData()->animid());
			m_pOwner->SetCharacterStateStartAnimationPosition(packet_data->GetData()->playeraction()->positionlogic());
		...
		{
			auto packet_data = (CFlatBufPacket<F4PACKET::play_c2s_playerTapPass_data>*)pData;
			...
			m_pOwner->SetCharacterStateAction(*packet_data->GetData()->playeraction(), F4PACKET::ACTION_TYPE::action_tapPass, time);
			m_pOwner->SetCharacterStateAnimationIndex(packet_data->GetData()->animid());
			m_pOwner->SetCharacterStateStartAnimationPosition(packet_data->GetData()->playeraction()->positionlogic());
		...
		{
			auto packet_data = (CFlatBufPacket<F4PACKET::play_c2s_playerJumpBallTapOut_data>*)pData;
			...
			m_pOwner->SetCharacterStateAction(*packet_data->GetData()->playeraction(), F4PACKET::ACTION_TYPE::action_jumpBallTapOut, time);
			m_pOwner->SetCharacterStateAnimationIndex(packet_data->GetData()->animid());
			m_pOwner->SetCharacterStateStartAnimationPosition(packet_data->GetData()->playeraction()->positionlogic());

			m_SlideTimePosition = m_pOwner->GetAnimationController()->GetEventTime(packet_data->GetData()->animid(), "ball_out");
			m_AnimationDuration = m_pOwner->GetAnimationController()->GetAnimationDuration(packet_data->GetData()->animid());
			// 왜 그런지는 모르겠는데 점프볼할 때는 무조건 미러가 true 임
			m_Mirror = packet_data->GetData()->ismirroranim();
			m_Success = packet_data->GetData()->success();
...
void CCharacterState_Rebound::OnExit()
{

}

void CCharacterState_Rebound::Init()
{
	m_Mirror = false;
	m_Success = false;
...
DHOST_TYPE_BOOL CCharacterState_Rebound::GetSecondAnimationReceivePacket()
{
	return false;
}
    ...
    for (result = resultList; result != NULL; result = result -> ai_next)
    {
        if (result->ai_addr != NULL && result->ai_addrlen >= sizeof(struct sockaddr_in))
        {
            if (result->ai_family == AF_INET)
            {
                ...
                ((UINT32 *)& address->host.s6_addr)[0] = 0;
                ((UINT32 *)& address->host.s6_addr)[1] = 0;
                ((UINT32 *)& address->host.s6_addr)[2] = htonl(0xffff);
                ((UINT32 *)& address->host.s6_addr)[3] = sin->sin_addr.s_addr;
